...
## Tutorial
PACKAGE_MANAGER это apt
```sh
$ export GITHUB_USERNAME=<имя_пользователя>
$ export PACKAGE_MANAGER=<пакетный_менеджер>
```
При попытке ввести вторую команду терминал выдавал ошибку, т.к. не было рут-прав. Поэтому пишем
```
...
EOF
```
vagrant-vbguest - это плагин, который автоматически обновляет гостевые дополнения VirtualBox. Настройка виртуальной машины:
```sh
$ cat >> Vagrantfile <<EOF
...
EOF
```
Продолжение настройки виртуальной машины:
```sh
$ cat >> Vagrantfile <<EOF
  ...
  config.vm.provider "virtualbox" do |vb|                         # Указываем тип виртуальной машины: virtualbox
    ...
    vb.memory = "2560"                                            # Указываем, сколько выделяем оперативной памяти под виртуальную машину: 2560МБ 
  end
...
```
Здесь терминал опять сломался, т.к. у меня не было библиотеки libvirt.В документации я нашёл порядок действий, необходимых для установки этой библиотеки. Сначала нужно проверить, поддерживает ли вообще мой процессор виртуализацию:
```
$ kvm-ok
...
$ vagrant ssh                            # подключение к виртуалке через ssh
  System information as of Tue 15 Jun 2021 04:30:51 PM UTC
...
==> default: Snapshotting the machine as 'push_1240543164_4159'...
	==> default: Snapshot saved! You can restore the snapshot at any time by
	==> default: using `vagrant snapshot restore`. You can delete it using
	==> default: `vagrant snapshot delete`.
$ vagrant snapshot list             
==> default: 
	push_1240543164_4159
$ vagrant halt                           # выключение виртуалки
 ==> default: Attempting graceful shutdown of VM...
$ vagrant snapshot pop                   # открытие снимка виртуалки
	==> default: Restoring the snapshot 'push_1240543164_4159'...
	==> default: Deleting the snapshot 'push_1240543164_4159'...
